...
aabbbbbaabbbaaaaaabbbbbababaaaaabbaaabba"""
        ...
        val solver = PregeneratingMatcher(rules.lines())

        assertTrue(solver.matches("bbabbbbaabaabba"))
        assertTrue(solver.matches("babbbbaabbbbbabbbbbbaabaaabaaa"))
        assertTrue(solver.matches("aaabbbbbbaaaabaababaabababbabaaabbababababaaa"))
        assertTrue(solver.matches("bbbbbbbaaaabbbbaaabbabaaa"))
        assertTrue(solver.matches("bbbababbbbaaaaaaaabbababaaababaabab"))
        assertTrue(solver.matches("ababaaaaaabaaab"))
        assertTrue(solver.matches("ababaaaaabbbaba"))
        assertTrue(solver.matches("baabbaaaabbaaaababbaababb"))
        assertTrue(solver.matches("abbbbabbbbaaaababbbbbbaaaababb"))
        assertTrue(solver.matches("aaaaabbaabaaaaababaa"))
        assertTrue(solver.matches("aaaabbaabbaaaaaaabbbabbbaaabbaabaaa"))
        assertTrue(solver.matches("aabbbbbaabbbaaaaaabbbbbababaaaaabbaaabba"))
        assertFalse(solver.matches("abbbbbabbbaaaababbaabbbbabababbbabbbbbbabaaaa"))
        assertFalse(solver.matches("aaaabbaaaabbaaa"))
        assertFalse(solver.matches("babaaabbbaaabaababbaabababaaab"))

        assertEquals(12, messages.lines().count(solver::matches))
    }
